...
public class ChaBoShiCallBack extends HttpServlet {
	...
	/*public static void main(String[] args) {
		Map<String,String> result = CBS.getInstance(QueryAppKeyLib.baoyangUserId,QueryAppKeyLib.baoyangUserKey).getNewReportUrl("112ae64616d940a89f8bc607bdb1dd02");
		System.out.println(result.get("mobileUrl").replace("http:", "https:"));
    }*/
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result!=null){
			...
			Map<String,String> resulturl = CBS.getInstance(QueryAppKeyLib.baoyangUserId,QueryAppKeyLib.baoyangUserKey).getNewReportUrl(orderid);
			WXJL w = gson.fromJson(callResult, WXJL.class);
			if(resulturl!=null && resulturl.get("mobileUrl")!=null){
				w.setWapurl(resulturl.get("mobileUrl").replace("http:", "https:"));
			}
			String isUseBaoYangJiLuWapPage = PropertiesUtils.getPropertyValueByKey("isUseBaoYangJiLuWapPage");
			if("1".equals(isUseBaoYangJiLuWapPage)){
				w.setIsUseBaoYangJiLuWapPage("1");
			}else{
				w.setIsUseBaoYangJiLuWapPage("0");
			}
			w.translateWBJL(w);
			payService.updateBYJLFinancePayContent(w.getVin(),JsonUtils.toJson(w));
			logger.info("callResult:\r\n"+callResult);
			logger.info("callResult-mobileUrl:\r\n"+resulturl.get("mobileUrl"));
	...
	public static void main(String[] args) {
	  ...
	  Map<String,String> resulturl = CBS.getInstance(QueryAppKeyLib.baoyangUserId,QueryAppKeyLib.baoyangUserKey).getNewReportUrl("112ae64616d940a89f8bc607bdb1dd02");
	  if(resulturl!=null && resulturl.get("mobileUrl")!=null){
			w.setWapurl(resulturl.get("mobileUrl").replace("http:", "https:"));
		}
		w.translateWBJL(w);
